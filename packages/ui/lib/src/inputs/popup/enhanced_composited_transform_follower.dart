...
class EnhancedRenderFollowerLayer extends RenderProxyBox {
    ...
    double adjust({
      ...
      if (flipWhenOverflow) {
        ...
        if (position + size > maxBoundary) {
          adjustedPosition = altPosition - size >= minBoundary ? altPosition - size : maxBoundary - size;
        } else if (position < minBoundary) {
          adjustedPosition = altPosition + altSize <= maxBoundary ? altPosition + altSize : minBoundary;
        }
      }
  ...
  bool hitTest(BoxHitTestResult result, {required Offset position}) =>
      ...
      (link.leader == null && !showWhenUnlinked) ? false : hitTestChildren(result, position: position);
