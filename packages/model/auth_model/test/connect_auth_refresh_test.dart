...
void main() {
  group('ConnectAuthenticationMiddleware reactive refresh', () {
    ...
    test('non-session-ending public path (Authenticate): UNAUTHENTICATED is a flow error, NOT a logout', () async {
      ...
      final mw = ConnectAuthenticationMiddleware(
        ...
        onAuthError: () => loggedOut = true,
        unauthenticatedPaths: const {'/auth.v1.AuthService/Authenticate', kAuthServiceRefreshTokensPath},
        sessionEndingPaths: const {kAuthServiceRefreshTokensPath},
      );

      await expectLater(
        mw.handle(invoker)('/auth.v1.AuthService/Authenticate', <String, String>{}),
        throwsA(isA<ConnectException>()),
      );
  ...
  group('ConnectAuthenticationMiddleware streaming (repair without replay)', () {
