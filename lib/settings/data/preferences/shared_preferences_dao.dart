...
abstract base class PreferencesDao {
  ...
  const PreferencesDao(this._sharedPreferences);
  ...
  PreferencesEntry<double> doubleEntry(String key) =>
      _PreferencesEntry<double>(key: key, sharedPreferences: _sharedPreferences);

  /// Obtain [int] entry from the preferences.
  PreferencesEntry<int> intEntry(String key) => _PreferencesEntry<int>(key: key, sharedPreferences: _sharedPreferences);

  /// Obtain [String] entry from the preferences.
  PreferencesEntry<String> stringEntry(String key) =>
      _PreferencesEntry<String>(key: key, sharedPreferences: _sharedPreferences);

  /// Obtain [Iterable<String>] entry from the preferences.
  PreferencesEntry<Iterable<String>> iterableStringEntry(String key) =>
      _PreferencesEntry<Iterable<String>>(key: key, sharedPreferences: _sharedPreferences);
}
...
final class _PreferencesEntry<T extends Object> extends PreferencesEntry<T> {
  ...
  Future<void> set(T value) async {
    await switch (value) {
      ...
      final Iterable<String> v => _sharedPreferences.setStringList(key, v.toList()),
      _ => throw Exception('$T is not a valid type for a preferences entry value.'),
    };
  }
