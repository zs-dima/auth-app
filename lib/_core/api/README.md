...
- **Transport-generic middleware lives in packages** — it has no app dependencies and is reusable:
  ...
  - `auth_model` → `GrpcAuthenticationMiddleware` and `HttpAuthenticationMiddleware` (token attach +
    single-flight refresh). The HTTP auth middleware is a tested mirror of the gRPC one but is **not
...
- **Observability middleware lives here, in `lib`** — it depends on app-only concerns and would force
  ...
  - `grpc/` + `http/` sentry middleware → depend on `sentry_flutter` and the app's Sentry config (DSN,
    environment). Keeping them here means `http_client`/`grpc_model` stay free of `sentry_flutter` and
